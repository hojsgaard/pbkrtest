...
First we define a `caracas` symbol `x` (see the appendix) and 
...
The determinant of $M$ is $det(M)=`r det(M)`$ and this can be factored out of the matrix by dividing each entry with the determinant and multiplying the new matrix by the determinant which simplifies the appearence of the matrix:

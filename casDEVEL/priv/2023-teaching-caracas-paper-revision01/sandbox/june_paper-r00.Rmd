...
package [@ryacas] enhance the capability of `R` [@R] to handle
symbolic mathematics. In this paper we will illustrate the use of the
`caracas` package in connection with teaching mathematics and
...
is available from CRAN [@R].  The open-source development version of
...
the symbol in `R` and in `Python`. 


## Documents with mathematical content

A LaTeX rendering of a `caracas` symbol, say `x` is obtained by typing
`` $$x = `r knitr::inline_expr("tex(x)")`$$ ``. This feature is useful when creating documents with a mathematical content and has been used extensively throughout this paper (looks nice and saves space).
...
$$
M = `r tex(M)`; \quad M^{-1} = `r tex(Minv_fact)` = `r tex(Minv)` .
$$
...
A vector is a one-column matrix which is printed as its transpose to save space. Matrix products are computed using the `%*%` operator:
...
$$
 grad  = `r tex(grad)`; \quad 
 grad2  = `r tex(grad2)`; \quad  
$$

In a more general setting we can find the stationary points by equating the gradient to zero:
The output `sol` is a list of list of `caracas` symbols. 
...
## Low-level access to engines [TBW: MMA]

[FIXME] Need something here; also on calling Ryacas



Here we return to the example in the [Calculus] section 
with roots of a polynomial derivative. 
This example both illustrates how to use SymPy directly, 
but also highlights why we made the `caracas` package: 
this is a lot of unclear low-level code making it both 
...
This can also be done entirely without `caracas` 
using the `reticulate` package [@reticulate], 
but as e.g. the mathematical operators (e.g. `+`) 
are not defined, we either have to do everything for example along these lines:
...
Remember to refer to SymPy's elaborate documentation 
at <https://docs.sympy.org>.

Another example is from linear algebra:

```{r}
library(caracas)
A <- matrix_(c("a", "0", "a", "a"), nrow = 2)
A
inv(A)
tex(inv(A))
```
...
A_str <- "Matrix([[a, a], [0, a]])" 
A <- py_eval(A_str, convert = FALSE) 
A
A$inv() 
s$latex(A$inv())
```

As seen, using SymPy directly can be powerful, but it also 
involves knowing much more Python and SymPy internals and 
these requirements steepens the learning curve for both writing and 
reading the code.


## Access to Ryacas and other engines [TBW: MMA/SH]

Notice that some operations can in some circumstances be performed 
faster with other back-ends than SymPy. Matrix inversion is one such 
example.  

Therefore `inv()` takes a `method` argument that can be 
used to specify what method to use (currently LU, Gaussian elimination, 
or the cofactor from SymPy or `Ryacas`'s method). 
We expect that more such examples will arise, 
which will further improve the usability of `caracas`.

<!---

```{r}
# A <- as_sym(toeplitz(c("a", "b", "c")))
# A
# inv(A)
# 
# microbenchmark::microbenchmark(
#   lu = inv(A, method = "lu"),
#   gauss = inv(A, method = "gauss"),
#   cf = inv(A, method = "cf"),
#   Ryacas = inv(A, method = "yac"), 
#   times = 10
# )
```
--->

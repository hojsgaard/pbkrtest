...
The paper is organized as follows:\footnote{FINISH LATER} Sec. \ref{sec:primer} contains a
...
```{r, echo=FALSE}
get_pyobj <- function(object, ...){ ## WANT THIS?
    object[[1]]
}
```
...
as_func(hessian) (stationary_points)
```
...
```{r, results='hide'}
sol <- solve_sys(grad, x)
lapply(sol, function(s) subs(hessian, s)) 
```
...
### The total likelihood {#sec:logistic-all-likelihood}
...
par <- optim(c(a=0, v=1), f_wrap, lower=c(-.99, .01), upper=c(.99, 10),
...
## A compound symmetry covariance structure - and some limitations of `caracas` {#sec:cov_fun}
...
on the diagonal and $v r$ outside the diagonal. That is, for $n=`r n`$:
...
# Possible topics and smaller projects for students
...
1. Related to Sec.\ \ref{sec:ar1}: Find (approximate) stanard error

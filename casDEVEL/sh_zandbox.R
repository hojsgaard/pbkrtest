...
update_sym <- function(sym, nms, vls){
    dict <- paste0("{", paste0(nms, ": ", vls, collapse=", "), "}")
    cat("dict: \n"); print(dict)
    e <- reticulate::py_eval(dict)
    construct_symbol_from_pyobj(sym$pyobj$subs(e))
}

as.char <- function(x, ...){ ## JUST A WRAPPER
...
any_to_char <- function(x){
    if (is_sym(x) && (
        symbol_is_vector(x) || symbol_is_atomic(x))){
        nms_ <- as.char(x)
    } else {
        nms_ <- as.char(x)
    }
...
## case: list
## nms <- list(xx = xx + yy, zz = yy^2 - xx) ## FAILS
nms <- list(xx = xx + yy, zz = yy - xx)
...
do.call(update_sym,
        c(sym=list(sym), list_to_nms_vls(nms)))
...
do.call(update_sym,
        c(sym=list(sym), list(nms=nms_, vls=vls_)))


## Case vector subsitution
p <- vector_sym(3, "p")
y <- vector_sym(3, "y")
sym <- sum(y * log(p))
...
nms_ <- c(as_character_matrix(nms))
vls_ <- c(as_character_matrix(vls))
list(nms=nms_, vls=vls_)
vls_[1] = "A"
vls_[2] = "7"

update_sym(sym, nms_, vls_)
...
update_sym(sym, as.char(nms), vls)



subs2 <- function(sym, nms, vls){
    if (inherits(nms, "list")){
...
## if (is_sym(nms) && (
    ## symbol_is_vector(nms) || symbol_is_atomic(nms))){
    ## nms_ <- as.char(nms)
## } else {
    ## nms_ <- as.char(nms)
## }
...
## if (is_sym(vls) && (
    ## symbol_is_vector(vls) || symbol_is_atomic(vls))){
    ## vls_ <- as.char(vls)
## } else {
    ## vls_ <- as.char(vls)
## }
...
subs_lst(p, list("p1" = "A", "p2" = y[1], "p3" = 0))

subs(p, p, y)
subs(p, p, y2)

update_sym(sym, nms, vls)

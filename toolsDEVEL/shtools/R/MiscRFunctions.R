...
wrapPlot <- function(expr, file="Rfigure", path=".\\",type="epspdf",width=6,height=6){
  full.path       <- paste(path,file,sep="")
  full.path.eps   <- paste(path,file,".eps",sep="")
  full.path.pdf   <- paste(path,file,".pdf",sep="")
  #print(full.path.eps)
  postscript(full.path.eps, width=width,height=height,horizontal=TRUE)
  eval(expr)
  dev.off()
  pdf(full.path.pdf, width=width,height=height)
  eval(expr)
  dev.off()
  #system(paste("epstopdf", full.path.eps))
}

bwrapPlot <- function(file="Rfigure", path=".\\",type="epspdf",width=6,height=6){
  full.path       <- paste(path,file,sep="")
  full.path.eps   <- paste(path,file,".eps",sep="")
  full.path.pdf   <- paste(path,file,".pdf",sep="")
  #print(full.path.eps)
  postscript(full.path.eps, width=width,height=height,horizontal=FALSE)
  return(full.path.eps)
}

ewrapPlot <- function(full.path.eps){
  dev.off()
  system(paste("epstopdf", full.path.eps),invisible=TRUE)
}

bPlot <- function(file="Rfigure", path=".\\",type="epspdf",width=6,height=6){
  full.path       <- paste(path,file,sep="")
  full.path.eps   <- paste(path,file,".eps",sep="")
  full.path.pdf   <- paste(path,file,".pdf",sep="")
  #print(full.path.eps)
  postscript(full.path.eps, width=width,height=height,horizontal=FALSE)
  ..full.path.eps <<-full.path.eps
  return(full.path.eps)
}

ePlot <- function(full.path.eps=NULL){
  dev.off()
  if (is.null(full.path.eps))
    system(paste("epstopdf", ..full.path.eps),invisible=TRUE)
  else
    system(paste("epstopdf", full.path.eps),invisible=TRUE)
}

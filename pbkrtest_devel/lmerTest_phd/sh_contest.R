...
contestMD.lmerModLmerTest <- function(model, L, rhs=0,
    ...
    } else { ## method == "Satterthwaite" proceeds:
...
get_sddf <- function(model, L, eps=sqrt(.Machine$double.eps)){
    ...
    nu_m <- vapply(1:q, function(m) {
        2*(d[m])^2 / qform(grad_PLcov[[m]], model@vcov_varpar) }, numeric(1L)) # 2D_m^2 / g'Ag
                                        # Compute ddf for the F-value:
    ddf <- get_Fstat_ddf(nu_m, tol=1e-8)
    list(Fvalue=Fvalue, ndf=q, ddf=ddf, sigma=model@sigma)
    ##ddf
} 

SATmodcomp <- function(model, L, eps=sqrt(.Machine$double.eps)){
    ...
    ## tmp <<- list(q=q, PtL=PtL, aux=aux, d=d)
    ...
    nu_m <- vapply(1:q, function(m) {
        2*(d[m])^2 / qform(grad_PLcov[[m]], aux$vcov_varpar)
                                        ...
                                        # Compute ddf for the F-value:
    ddf <- get_Fstat_ddf(nu_m, tol=1e-8)
    list(Fvalue=Fvalue, ndf=q, ddf=ddf, sigma=getME(model, "sigma"))
